...
void fft_pointwise(complex<double> *T, const complex<double> *A, int k) // T:  Pointer to array. k: 2^k is the size of the transform
{
...
void int_to_fft(complex<double> *T, int k, const uint32_t *A, size_t AL)
{
	...
	//  length of the input.
	if (fft_length < 3 * AL) throw "FFT length is too small.";
	...
	while (T < Tstop) *T++ = complex<double>(0, 0);
}
/*
参数：
	T  ： FFT array
